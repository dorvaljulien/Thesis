...
Many N-body integrators use a set of units specifically invented for the Nbody gravitational problem, the Nbody units, or H\'enon units (as prescribed by Douglas Heggie during the MODEST 2014 meeting). They were introduced by \cite{Henon1971} and are based on three relations:
...
\item[\textbf{Ahmad-Cohen Neighbour scheme;}] considering close neighbours and distant particles have distinct dynamical effects, their influence was split into regular and irregular integrations, with some level of synchronization;
...
\item Binaries - detect binary stars using the algorithm described in chapter \ref{Chap:BinAlgo};
...
\item Clump detection - extract overdensities through the algorithm described in next section;
